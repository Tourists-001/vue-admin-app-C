...
export default {
  ...
  methods: {
    ...
    scrollTop(i, e) {
      ...
      const itemLeft = e.target.getBoundingClientRect().left; // 距离左侧的宽度
      const wrapperWidth = oneTab.offsetWidth; // 中心距离左侧的宽度
      moveTo(oneTab.scrollLeft, itemWidth / 2 + itemLeft - wrapperWidth / 2, oneTab, 'scrollLeft');
      // this.$store.dispatch('getSideList', this.menuList[i].title); // 获取数据
